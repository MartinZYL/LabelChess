    ...
    for (int i = 0; i < Count; i ++) {
        ...
        for (int j = 0; j < Count; j ++) {
            ...
            if (i == 1 || i == 6) {
                ...
                //保证只在创建第七行label才进行 -- 的操作
                if (i == 7) {
                    k --;
                }
            } else {
                //创建空白的没有棋子的格子
                label = [LabelFactory createLabelWithTitle:nil frame:CGRectMake(j * Width, i * Width, Width, Width) side:noSide];
            }
            //设置格子的背景颜色
